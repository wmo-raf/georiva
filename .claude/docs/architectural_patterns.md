...
Two registries share the same structure: a class with `register()`, `get()`, and a module-level singleton
instance.
...
> **Note:** There is **no** analysis operator registry. The `analysis/` app ships two concrete,
> directly-wired modules (`timeseries/`, `zonal_stats/`) — not a generic `OperatorRegistry`. Any
> earlier reference to `analysis/registry.py` is stale; that file does not exist.
...
> **Proposed extension:** a declarative parameter-manifest contract (`describe_parameters()` returning
> a `ParameterManifest`) so plugins can list their parameters and combined products for catalog
> provisioning. See `docs/architecture/plugin-parameter-contract.md`.
...
- `INCOMING` (`georiva-incoming`) and `SOURCES` (`georiva-sources`) carry `s3:ObjectCreated:*`
  notifications that trigger ingestion.
- `ARCHIVE` (`georiva-archive`) preserves raw inputs; `ASSETS` (`georiva-assets`) is public-read and
  serves processed COGs/PNGs; `ZARR` (`georiva-zarr`) holds virtual-Zarr (kerchunk) manifests.
...
- Returns a rich result object: `IngestionResult` dataclass (`ingestion/result.py:14`)

Other stateless services follow the same shape: `analysis/timeseries/service.py` (`TimeseriesService`)
and the `analysis/zonal_stats/service.py` functions.
...
The same locking + sweep pattern is reused by `virtual_zarr/models.py` — `VirtualZarrManifest`
(`pending → building → ready → stale → failed`, `LOCK_TIMEOUT`, `reset_stale_locks()`,
`get_buildable()`).

**Convention**: Ingestion tasks do not use Celery retries (`max_retries=0` in `ingestion/tasks.py:23`). Recovery is
handled by the `sweep_unprocessed` periodic task (line 114) which resets stale locks, scans for untracked files, and
retries failures.
...
**Retries**: Ingestion tasks use `max_retries=0` (recovery via `sweep_unprocessed`). Newer, idempotent tasks use
bounded retries instead — e.g. `analysis/zonal_stats/tasks.py` (`compute_boundary_zonal_stats`, `max_retries=3`).
...
The virtual-Zarr layer extends this: `VirtualZarrManifest.open_dataset()` returns a lazy, dask-backed
`xarray.Dataset` that references COG chunks in place (kerchunk), which the time-series service reads.
...
| zonal_stats   | `analysis/zonal_stats/wagtail_hooks.py` | admin integration for boundary stats                                                                                 |
| virtual_zarr  | `virtual_zarr/wagtail_hooks.py`         | snippet/admin integration for manifests                                                                              |
...
- **Collection** (`core/models/collection.py`): Dataset within catalog. Owns temporal config (time_resolution, forecast
  settings) and `boundary_stats_levels`. Links to a `LoaderProfile` for ingestion.
- **Variable** (`core/models/variable.py`): Measured quantity. Maps raw source bands via a `sources` StreamField
  (`primary` / `u_component` / `v_component` `SourceBlock`s) plus a `transform_type` (`PASSTHROUGH`,
  `VECTOR_MAGNITUDE`, `VECTOR_DIRECTION`) — i.e. it supports both passthrough and derived (vector) variables.
- **Item** (`core/models/item.py`): Single spatiotemporal granule. TimescaleDB hypertable keyed on `time`
  (valid_time), with `reference_time` for forecasts.
- **Asset** (`core/models/item.py`): Physical file (COG, PNG, JSON) linked to an Item.

**Convention**: Models split into separate files under `core/models/`, re-exported from `__init__.py`.

## 12. Settings Split Pattern

`config/settings/base.py` → `dev.py` → `production.py`

- **base.py**: All shared config, reads `.env` via `django-environ`
...
**Storage backends**: Dynamically registered per-bucket in a loop over `GEORIVA_BUCKETS`, supporting both S3 and local
filesystem.
...
config/urls.py  →  api/urls.py  →  stac/urls.py
                                →  edr/urls.py
                                →  analysis/urls.py → analysis/timeseries/urls.py
                                →  task_ferry (jobs) / tile-config / datasets
```

- Each API app defines `app_name` for namespacing (e.g., `stac/urls.py:5`)
- RESTful nesting for STAC
- Wagtail catch-all at the end of `config/urls.py`
- MinIO events are **not** an HTTP route — they arrive via a Redis list consumed by `minio-consumer`.
...
|---------------------|--------------------------|---------------------------------------------|
| `IngestionResult`   | `ingestion/result.py:14` | File processing outcome with error tracking |
...
for derived values (e.g., `size_reduction_percent` at `ingestion/result.py:47`).
