...
class BoundaryClipper:
    ...
    def compute_window(
        ...
        # Check intersection
        if clip_east <= src_west or clip_west >= src_east or clip_north <= src_south or clip_south >= src_north:
            raise ValueError(f"Boundary bbox {self.bbox} does not intersect source bounds {src_bounds}")
        ...
        self._logger.debug(f"Clip window: ({x_off}, {y_off}) {win_width}x{win_height} from {src_width}x{src_height}")
    ...
    def create_mask(
        ...
        if cache_key not in self._mask_cache:
            ...
            self._mask_cache[cache_key] = ~geometry_mask(
                [self.shapely_geom], out_shape=(height, width), transform=transform, invert=False
            )

            self._logger.debug("Mask computed and cached for bounds=%s size=%dx%d", bounds, width, height)
