...
class SweepStagingTests(TestCase):
    def setUp(self):
        ...
        self.scol = StagingCollection.objects.create(catalog=self.catalog, slug="chirps-monthly", name="m")
    ...
    def _run_with_bucket(self, keys):
        with (
            ...
            mock_storage.bucket.return_value.list_files.return_value = [{"path": k} for k in keys]
    ...
    def test_registers_only_objects_without_a_staging_item(self):
        ...
        mock_task.run.assert_called_once_with(bucket=BucketType.STAGING, key="chirps/chirps-monthly/b.tif")
