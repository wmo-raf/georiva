...
def _scan(listing_by_bucket, prefix=None):
    ...
    def bucket_for(bucket_type):
        bucket = MagicMock()
        bucket.list_files.return_value = [{"path": p} for p in listing_by_bucket.get(bucket_type, [])]
        return bucket
...
class FindUnprocessedClassificationTests(TestCase):
    def test_classifies_untracked_pending_and_reingest_and_skips_in_flight(self):
        FileIngestion.objects.create(
            ...
            file_path="cat/col/pending.tif",
            status=FileIngestion.Status.PENDING,
        )
        FileIngestion.objects.create(
            bucket=BucketType.SOURCES,
            file_path="cat/col/reingest.tif",
            status=FileIngestion.Status.COMPLETED,
            force_reingest=True,
        )
        FileIngestion.objects.create(
            bucket=BucketType.SOURCES,
            file_path="cat/col/busy.tif",
    ...
    def test_force_reingest_wins_regardless_of_status(self):
        ...
        FileIngestion.objects.create(
            bucket=BucketType.SOURCES,
            file_path="cat/col/forced-failed.tif",
            status=FileIngestion.Status.FAILED,
            force_reingest=True,
        )
        FileIngestion.objects.create(
            bucket=BucketType.SOURCES,
            file_path="cat/col/forced-pending.tif",
            status=FileIngestion.Status.PENDING,
            force_reingest=True,
        )
        ...
        found = _scan(
            {
                BucketType.SOURCES: [
                    "cat/col/forced-failed.tif",
                    "cat/col/forced-pending.tif",
                ],
            }
        )
