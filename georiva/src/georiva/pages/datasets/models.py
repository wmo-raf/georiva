...
class DatasetsIndexPage(RoutablePageMixin, Page):
    ...
    intro_text = models.TextField(
        blank=True, verbose_name="Introduction", help_text="Optional intro shown above the dataset listing."
    )
    ...
    def catalog_detail(self, request, catalog_slug):
        ...
        collections = self._base_collections_qs(request).filter(catalog=catalog)

        return render(
            request,
            "datasets/catalog_detail.html",
            {
                "page": self,
                "catalog": catalog,
    ...
    def collection_detail(self, request, catalog_slug, collection_slug):
        ...
        variables = list(collection.variables.filter(is_active=True).order_by("sort_order"))
        ...
        return render(
            request,
            "datasets/collection_detail.html",
            {
                "page": self,
                "catalog": catalog,
    ...
    def collection_item_detail(self, request, catalog_slug, collection_slug, item_id):
        ...
        cog_assets = list(
            item.assets.filter(format=Asset.Format.COG, variable__is_active=True)
        ...
        texture_urls = {a.variable.slug: titiler_encoded_preview_url(item, a.variable) for a in cog_assets}
        ...
        next_item = Item.objects.filter(collection=collection, time__gt=item.time).order_by("time").values("pk").first()
        ...
        return render(
            ...
            "datasets/item_detail.html",
            {
                ...
                "martin_boundary_stats_url": martin_boundary_stats_url(
                    collection,
                    base=get_full_url_by_request(request, MARTIN_PREFIX),
                ),
            },
        )
    ...
    def _catalog_filter_context(self, request):
        ...
        active_resolutions = (
            org_collections.exclude(time_resolution="").values_list("time_resolution", flat=True).distinct()
        )
        ...
        return {
            ...
            "time_resolutions": [
                (value, choices[value]) for value in Collection.TimeResolution.values if value in active_resolutions
            ],
        }
