...
class ItemLineageTests(TestCase):
    def test_lineage_returns_the_source_items_of_a_produced_item(self):
        ...
        DerivationLink.objects.create(
            ...
            source_staging_item=staging_src,
            recipe_id="climatology",
            recipe_version="1",
            input_hash="h",
        )
        DerivationLink.objects.create(
            derived_item=derived,
            source_published_item=published_src,
            recipe_id="climatology",
            recipe_version="1",
            input_hash="h",
        )
...
class ChainViewTests(TestCase):
    ...
    def test_chain_page_renders_nodes_and_edge_labels(self):
        ...
        with patch.object(DataFeed, "get_derived_products", return_value=[_definition()]):
            response = self.client.get(reverse("derived_product_chain", kwargs={"feed_pk": self.feed.pk}))
    ...
    def test_chain_page_labels_the_product_states(self):
        ...
        with patch.object(DataFeed, "get_derived_products", return_value=[_definition(), promotion]):
            response = self.client.get(reverse("derived_product_chain", kwargs={"feed_pk": self.feed.pk}))
...
class ItemLineageViewTests(TestCase):
    ...
    def test_lineage_page_lists_a_produced_items_inputs(self):
        ...
        response = self.client.get(reverse("item_lineage", kwargs={"item_pk": derived.pk}))
